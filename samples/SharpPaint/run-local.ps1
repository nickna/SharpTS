...
$aotHost = Join-Path $root 'src\SharpTS.Gui.Host\bin\Release\net10.0\aot-publish'
dotnet publish (Join-Path $root 'src\SharpTS.Gui.Host\SharpTS.Gui.Host.csproj') -c Release --self-contained false --no-restore -p:SharpTSGuiHostLibrary=true -o $aotHost
