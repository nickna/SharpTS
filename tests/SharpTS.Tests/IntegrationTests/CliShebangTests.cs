...
public class CliShebangTests
{
    ...
    public void ExecutableTextStatsExample_RunsThroughShebangWithArguments()
    {
        ...
        var scriptPath = FindRepositoryFile("samples", "text-stats.ts");
        Assert.True(
            (File.GetUnixFileMode(scriptPath) & UnixFileMode.UserExecute) != 0,
            "samples/text-stats.ts must be committed with its executable bit set");
        ...
        Assert.Contains("Usage: ./samples/text-stats.ts", help.StandardOutput);
