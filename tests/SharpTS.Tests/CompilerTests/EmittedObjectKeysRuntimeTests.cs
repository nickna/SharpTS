    ...
    private static object MakeInputs(Type type, EmittedRuntime runtime, IReadOnlyDictionary<string, object?> overrides)
    {
        ...
        object? Value(ParameterInfo parameter)
        {
            ...
            return name switch
            {
                ...
                _ => (name switch { "TSFunctionType" => runtime.FunctionValues.Type, _ => (name switch { "InvokeMethodUnwrapped" => runtime.ReflectedMethods.InvokeUnwrapped, _ => typeof(EmittedRuntime).GetProperty(name)!.GetValue(runtime) }) })
            };
        }
