    ...
    private static object MakeInputs(Type type, EmittedRuntime runtime, IReadOnlyDictionary<string, object?> overrides)
    {
        ...
        object? Value(ParameterInfo parameter)
        {
            ...
            return name switch
            {
                ...
                _ => (name switch { "BoundTSFunctionType" => runtime.FunctionBindings.BoundType, "TSFunctionType" => runtime.FunctionValues.Type, _ => typeof(EmittedRuntime).GetProperty(name)!.GetValue(runtime) })
            };
        }
