...
public sealed class EmittedObjectWriteRuntimeTests
{
    ...
    private static object MakeInputs(Type type, EmittedRuntime runtime, IReadOnlyDictionary<string, object?> overrides)
    {
        ...
        return constructor.Invoke(constructor.GetParameters().Select(parameter =>
        {
            ...
            return (parameter.Name switch { "ArgumentsLengthField" => runtime.Arguments.LengthField, "ArgumentsType" => runtime.Arguments.Type, "BoundAnyFunctionType" => runtime.FunctionBindings.AnyType, "BoundTSFunctionType" => runtime.FunctionBindings.BoundType, "TSFunctionInvokeWithThis" => runtime.FunctionValues.InvokeWithThis, "TSFunctionType" => runtime.FunctionValues.Type, _ => (parameter.Name switch { "InvokeMethodUnwrapped" => runtime.ReflectedMethods.InvokeUnwrapped, "SafeGetMethod" => runtime.ReflectedMethods.FindMethod, _ => typeof(EmittedRuntime).GetProperty(parameter.Name!)!.GetValue(runtime) }) });
        }).ToArray());
    }
