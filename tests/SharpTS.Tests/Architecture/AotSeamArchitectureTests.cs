...
public class AotSeamArchitectureTests
{
    ...
    /// <summary>
    /// Enumerates the .cs sources compiled into SharpTS.dll from its bounded
    /// source directory.
    /// </summary>
    private static IEnumerable<(string FullPath, string Relative)> EnumerateProductSources()
    {
        string sourceRoot = Path.Combine(FindRepoRoot(), "src", "SharpTS");
    ...
    private static string FindRepoRoot()
    {
        ...
        {
            if (File.Exists(Path.Combine(dir.FullName, "SharpTS.sln")) &&
                File.Exists(Path.Combine(dir.FullName, ".gitmodules")))
                return dir.FullName;
            dir = dir.Parent;
        }
