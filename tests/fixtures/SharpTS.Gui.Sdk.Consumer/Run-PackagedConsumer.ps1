...
$supportedPlatformsPath = Join-Path $repositoryRoot "src\SharpTS.Gui.Sdk\Sdk\SupportedPlatforms.props"
...
$aotHostPublishRoot = Join-Path $repositoryRoot "src\SharpTS.Gui.Host\bin\Release\net10.0\aot-publish"
...
Invoke-DotNet @("restore", "src\SharpTS.Sdk.Tasks\SharpTS.Sdk.Tasks.csproj")
Invoke-DotNet @("restore", "src\SharpTS.Gui.Host\SharpTS.Gui.Host.csproj")
Invoke-DotNet @("build", "src\SharpTS.Sdk.Tasks\SharpTS.Sdk.Tasks.csproj", "-c", "Release", "--no-restore")
Invoke-DotNet @("publish", "src\SharpTS.Gui.Host\SharpTS.Gui.Host.csproj", "-c", "Release", "--self-contained", "false", "--no-restore")
...
if ($null -ne $candidatePackagePath) {
    ...
    Invoke-DotNet @("restore", "src\SharpTS.Gui.Sdk\SharpTS.Gui.Sdk.csproj")
    Invoke-DotNet @("pack", "src\SharpTS.Gui.Sdk\SharpTS.Gui.Sdk.csproj", "-c", "Release", "--no-restore", "-o", $feed, "-p:MinVerVersionOverride=$version")
}
...
$sharpTsCli = Join-Path $repositoryRoot "src\SharpTS\bin\Release\net10.0\SharpTS.dll"
