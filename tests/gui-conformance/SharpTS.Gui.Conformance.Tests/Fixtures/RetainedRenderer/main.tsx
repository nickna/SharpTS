...
let viewCount = 0;
let desktopRoot: DesktopWindow;
let testDriver: DesktopTestDriver;
...
failNextNativeSetter(desktopRoot, "native-probe");
setNativeValue(1);
...
traceControlIdentities(desktopRoot, "identities-initial");

function afterCoalescedUpdate(): void {
    trace("coalesced-update-complete");
    traceControlIdentities(desktopRoot, "identities-reordered");
    if (!transientRef.isAttached) {
...
function afterFinalUpdate(): void {
    testDriver.setTextBoxValue("name", "User");
    testDriver.setCheckBoxValue("enabled", false);
    testDriver.setComboBoxIndex("choice", 2);
    testDriver.setSliderValue("amount", 9);
    trace("forms-events-complete");
    trace("reactive-update-complete");
    traceControlIdentities(desktopRoot, "identities-final");
    if (lifecycleScenario === undefined)
        afterTrace("guest-timer", () => testDriver.click("action"));
}
