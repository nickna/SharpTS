...
public sealed class ProductizationContractTests
{
    ...
    public void SupportedPlatforms_DriveSdkAndAreCoveredByTheWorkflow()
    {
        ...
        string platformsPath = Path.Combine(root, "src", "SharpTS.Gui.Sdk", "Sdk", "SupportedPlatforms.props");
        ...
        string sdkTargets = File.ReadAllText(Path.Combine(root, "src", "SharpTS.Gui.Sdk", "Sdk", "Sdk.targets"));
        string packageProject = File.ReadAllText(Path.Combine(root, "src", "SharpTS.Gui.Sdk", "SharpTS.Gui.Sdk.csproj"));
        string harness = File.ReadAllText(Path.Combine(root, "tests", "fixtures", "SharpTS.Gui.Sdk.Consumer", "Run-PackagedConsumer.ps1"));
    ...
    {
        string root = FindRepositoryRoot();
        string packageRoot = Path.Combine(root, "src", "SharpTS.Gui.Sdk", "GuiPackage");
