...
public sealed class GeneratedControlContractTests
{
    ...
    public void GeneratedRuntimeSdkAndDocumentationHaveMatchingSemanticIdentityAndControlKinds()
    {
        ...
        string typescript = File.ReadAllText(Path.Combine(root, "src", "SharpTS.Gui.Sdk", "GuiPackage", "control-surface.generated.ts"));
        Assert.Contains(DesktopBridge.DescriptorSchemaHash, typescript, StringComparison.Ordinal);
        using JsonDocument docs = JsonDocument.Parse(File.ReadAllText(Path.Combine(root, "src", "SharpTS.Gui.Sdk", "GuiPackage", "control-docs.generated.json")));
...
#endif
        string generator = Path.Combine(root, "src", "SharpTS.Gui.Generator", "bin", configuration, "net10.0", "SharpTS.Gui.Generator.dll");
    ...
    public void CheckedInGeneratedOutputsUseUtf8WithoutBomAndCanonicalLf()
    {
        ...
        string[] paths =
        [
            Path.Combine(root, "src", "SharpTS.Gui", "Generated", "ControlContract.Generated.cs"),
            Path.Combine(root, "src", "SharpTS.Gui.Sdk", "GuiPackage", "control-surface.generated.ts"),
            Path.Combine(root, "src", "SharpTS.Gui.Sdk", "GuiPackage", "control-docs.generated.json"),
            Path.Combine(root, "src", "SharpTS.Gui.Sdk", "Sdk", "DescriptorContract.Generated.props"),
        ];
    ...
    private static string ManifestPath() => Path.Combine(FindRoot(), "src", "SharpTS.Gui", "Controls", "controls.v1.json");
