...
| --- | --- | --- |
| Files, paths, and process | `fs`, `fs/promises`, `path`, `path/posix`, `path/win32`, `os`, `process`, `tty`, `module` | ✅ Core sync/async filesystem operations, explicit path variants, process lifecycle and stdio, compiled promise-rejection lifecycle events, host-appropriate POSIX identity calls, real standalone `ppid`, built-in discovery, and scoped literal `createRequire`. |
| Data and utilities | `assert`, `assert/strict`, `buffer`, `console`, `crypto`, `querystring`, `string_decoder`, `url`, `util`, `util/types`, `v8`, `zlib` | ✅ Broad tested subsets. Crypto includes dual-mode KeyObject JWK/DER parity, ECDH conversion/agreement, one-shot Diffie-Hellman, and X.509 inspection; algorithms remain bounded by .NET. `v8` uses a SharpTS-private serialization format and approximate managed-heap statistics. |
...
| Streams and networking | `stream`, `stream/consumers`, `stream/promises`, `stream/web`, `http`, `https`, `net`, `tls`, `dgram`, `dns`, `dns/promises` | ✅ HTTP/TCP/TLS/UDP/DNS and stream subsets, including shared TypeScript `dns` and partial `net` facades, strict Node-style IP parsing, asynchronous callbacks, resolver cancellation, and consumers for already-queued Node/Web readables. Live pull-source consumption and a few platform facilities have documented deviations. |
| Processes and isolation | `child_process`, `cluster`, `worker_threads`, `vm` | ⚠️ Main APIs are available. `Worker` uses dedicated OS threads in both modes; eligible compiled workers use isolated collectible realms, while documented cross-realm gaps retain an interpreter-backed compatibility path. See the [worker contract](docs/worker-threads.md). The .NET model does not reproduce every Node isolation or resource-limit option. Some compiled operations require the co-located managed runtime and source payload. |
