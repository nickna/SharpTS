...
| Streams and networking | `stream`, `stream/consumers`, `stream/promises`, `stream/web`, `http`, `https`, `net`, `tls`, `dgram`, `dns`, `dns/promises` | ✅ HTTP/TCP/TLS/UDP/DNS and stream subsets, including a shared TypeScript DNS facade, asynchronous callbacks, resolver cancellation, and consumers for already-queued Node/Web readables. Live pull-source consumption and a few platform facilities have documented deviations. |
