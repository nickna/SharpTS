...
public sealed class CalculatorHeadlessTests
{
    ...
    private static async Task<TraceEvent[]> RunAsync(string mode)
    {
...
#endif
        string hostSource = Path.Combine(root, "SharpTS.Gui.Host", "bin", configuration, "net10.0");
        string conformanceRoot = Path.Combine(root, "SharpTS.Gui.Conformance.Tests", "obj", configuration, "net10.0", ".sharpts-gui-conformance");
        ...
        try
        {
            ...
            File.Copy(Path.Combine(root, "Examples", "Calculator", "CalculatorApp.tsx"), Path.Combine(guestDirectory, "CalculatorApp.tsx"), true);
            File.Copy(Path.Combine(root, "Examples", "Calculator", "calculator.ts"), Path.Combine(guestDirectory, "calculator.ts"), true);
