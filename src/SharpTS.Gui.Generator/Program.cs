...
string manifestPath = Path.Combine(root, "src", "SharpTS.Gui", "Controls", "controls.v1.json");
...
{
    [Path.Combine(root, "src", "SharpTS.Gui", "Generated", "ControlContract.Generated.cs")] = CanonicalText(GenerateCSharp(manifest, hash)),
    [Path.Combine(root, "src", "SharpTS.Gui.Sdk", "GuiPackage", "control-surface.generated.ts")] = CanonicalText(GenerateTypeScript(manifest, hash)),
    [Path.Combine(root, "src", "SharpTS.Gui.Sdk", "GuiPackage", "control-docs.generated.json")] = CanonicalText(GenerateDocumentation(manifest, hash)),
    [Path.Combine(root, "src", "SharpTS.Gui.Sdk", "Sdk", "DescriptorContract.Generated.props")] = CanonicalText(GenerateSdkProps(hash)),
};
...
static void Validate(JsonObject manifest)
{
    ...
    foreach ((string groupName, JsonNode? value) in manifest["propertyGroups"]!.AsObject())
    {
        if (value is not JsonArray group)
            throw new InvalidOperationException($"Property group '{groupName}' must be an array.");
        foreach (JsonObject prop in group.Select(item => item!.AsObject()))
            RequiredDocumentation(prop, $"property group '{groupName}' prop '{Required(prop, "name")}'");
    }
    ...
    foreach (JsonObject control in manifest["controls"]!.AsArray().Select(item => item!.AsObject()))
    {
        ...
        RequiredDocumentation(control, $"control '{kind}'");
        ...
        {
            RequiredDocumentation(prop, $"control '{kind}' prop '{Required(prop, "name")}'");
            if (!props.Add(Required(prop, "name"))) throw new InvalidOperationException($"Duplicate prop '{Required(prop, "name")}' on '{kind}'.");
        }
        foreach (JsonObject evt in control["events"]!.AsArray().Select(item => item!.AsObject()))
        {
            RequiredDocumentation(evt, $"control '{kind}' event '{Required(evt, "name")}'");
            if (!props.Add(Required(evt, "name"))) throw new InvalidOperationException($"Duplicate prop/event '{Required(evt, "name")}' on '{kind}'.");
        }
    }
}

static void RequiredDocumentation(JsonObject value, string subject)
{
    if (value["documentation"]?.GetValue<string>() is not { } documentation || string.IsNullOrWhiteSpace(documentation))
        throw new InvalidOperationException($"Missing documentation for {subject}.");
}
...
static string DescriptorExpression(string adapter) => adapter switch
{
    ...
    "toggle-button" => "new CheckBoxDescriptor(\"ToggleButton\")",
...
static string GenerateTypeScript(JsonObject manifest, string hash)
{
    ...
    text.AppendLine("/** @internal */\nexport interface ContentChildProps { children?: TextualChild | GuiElement; }");
    var generatedPropsTypes = new HashSet<string>(StringComparer.Ordinal);
    foreach (JsonObject control in manifest["controls"]!.AsArray().Select(item => item!.AsObject()))
    {
        string kind = Required(control, "kind");
        string propsType = Required(control, "propsType");
        if (!generatedPropsTypes.Add(propsType)) continue;
        string handle = Required(control, "handle");
        string childBase = control["children"]!["model"]!.GetValue<string>() switch { "many" => ", ChildrenProps", "text" => ", TextualChildrenProps", "singleElement" => ", SingleElementChildProps", "content" => ", ContentChildProps", _ => "" };
        ...
        text.AppendLine($"/** Properties accepted by the {kind} component.\n * @category Components\n */");
        text.AppendLine($"export interface {propsType}{bases} {{");
        foreach (JsonObject prop in control["props"]!.AsArray().Select(item => item!.AsObject()))
        {
            text.AppendLine($"    /** {Documentation(prop)} */");
            text.AppendLine($"    {Required(prop, "name")}{(prop["required"]?.GetValue<bool>() == true ? "" : "?")}: {Required(prop, "type")};");
        }
        foreach (JsonObject evt in control["events"]!.AsArray().Select(item => item!.AsObject()))
        {
            text.AppendLine($"    /** {Documentation(evt)} */");
            text.AppendLine($"    {Required(evt, "name")}?: {Required(evt, "type")};");
        }
        text.AppendLine("}");
    }
    ...
    {
        string kind = Required(control, "kind");
        text.AppendLine($"/** {Documentation(control)}\n * @category Components\n * @function\n */");
...
static string Documentation(JsonObject value) =>
    value["documentation"]!.GetValue<string>().Replace("*/", "* /", StringComparison.Ordinal).Trim();
...
static string GenerateDocumentation(JsonObject manifest, string hash)
{
    ...
    foreach (JsonObject source in manifest["controls"]!.AsArray().Select(item => item!.AsObject()))
    {
        ...
        controls.Add(new JsonObject
        {
            ...
            ["nativeType"] = Required(source, "nativeType"),
            ["propsType"] = Required(source, "propsType"),
            ["handle"] = Required(source, "handle"),
            ["children"] = source["children"]!.DeepClone(),
...
static JsonObject DocProp(JsonObject prop) => new()
{
    ...
    ["documentation"] = Documentation(prop),
    ["required"] = prop["required"]?.GetValue<bool>() == true,
