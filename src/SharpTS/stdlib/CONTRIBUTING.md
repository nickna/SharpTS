...
the `stdlib\**\*.ts` `EmbeddedResource` group in `src/SharpTS/SharpTS.csproj`. (The original
...
package root; resource names are pinned with `LogicalName` in `src/SharpTS/SharpTS.csproj`
...
8. Add test fixtures under `tests/SharpTS.Tests/SharedTests/BuiltInModules/` that
...
- **Behavior tests**: `tests/SharpTS.Tests/SharedTests/BuiltInModules/<Name>ModuleTests.cs`
  exercise the public API through both execution modes, asserting
  Node-compatible output.
- **Standalone tests**: `tests/SharpTS.Tests/CompilerTests/StandaloneDllTests.cs`
