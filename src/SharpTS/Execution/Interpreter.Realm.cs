...
public partial class Interpreter
{
        ...
        foreach (var name in globalFunctionNames)
        {
            if (!globals.ContainsKey(name))
                globals[name] = new SharpTSGlobalFunction(
                    name, name is BuiltInNames.BigInt or BuiltInNames.Eval
                        or BuiltInNames.EncodeURIComponent or BuiltInNames.DecodeURIComponent
                        ? 1
                        : 0);
        }
    ...
    internal bool TryGetRealmIntrinsic(string name, out object? value)
    {
        ...
        if (name == BuiltInNames.Promise)
        {
            value = PromiseGlobalValue;
            return true;
        }
        value = null;
        return false;
    ...
    internal static bool IsRealmIntrinsicName(string name)
        => name is "Object" or "Math" or "JSON" or "String" or "Number" or "Boolean" or "Array" or "RegExp" or "Promise"
