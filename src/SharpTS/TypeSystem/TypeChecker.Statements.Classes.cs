...
public partial class TypeChecker
{
    ...
    private void CheckClassDeclaration(Stmt.Class classStmt)
    {
        ...
        try
        {
            ...
            var (classTypeEnv, classTypeParams, superclass) = CreateClassTypeEnvironment(classStmt);

            CheckClassSuperclassDeclarationOrder(classStmt);
            ...
            {
                CollectClassIndexSignatures(classStmt, mutableClass);
            ...
            TypeInfo.Class classTypeForBody = PublishInitialClassType(classStmt, mutableClass, classTypeParams);

            ValidateDeclaredClassInterfaces(classStmt, classTypeForBody, classTypeParams);

            ValidateDeclaredClassMembers(classStmt, classTypeForBody, classTypeParams, classTypeEnv);

            bool anyInferredFieldTypeResolved = CheckClassStaticFieldInitializers(
                classStmt, classTypeForBody, mutableClass);

            CheckClassAutoAccessorInitializers(classStmt, classTypeForBody);

            CheckClassStaticBlocks(classStmt, classTypeForBody);

            TypeEnvironment classEnv = CreateClassBodyEnvironment(
                classStmt, classTypeForBody, classTypeParams, superclass);
            ...
            RepublishInferredClassType(
                classStmt, mutableClass, classTypeParams, anyInferredMethodReturnResolved, anyInferredFieldTypeResolved);
