...
public abstract partial class ExpressionEmitterBase
{
    private void EmitFunctionConstructor(List<Expr> arguments)
    {
        ...
        IL.Emit(OpCodes.Call, Ctx.Runtime!.FunctionValues.Construct);
        SetStackUnknown();
    }
        ...
        switch (className)
        {
            ...
            case "ArrayBuffer":
                if (arguments.Count == 0)
                    ...
                    EmitExpressionAsDouble(arguments[0]);
                IL.Emit(OpCodes.Call, Ctx.Runtime!.RequireArrayBuffer().Create);
                SetStackUnknown();
                return true;
    ...
    private void EmitNewRegExpConstructor(List<Expr> arguments)
    {
        ...
        switch (arguments.Count)
        {
            case 0:
                IL.Emit(OpCodes.Ldstr, "");
                IL.Emit(OpCodes.Ldstr, "");
                IL.Emit(OpCodes.Call, Ctx.Runtime!.RegExps.RequireImplementation().CreateWithFlags);
                break;
            case 1:
                EmitExpression(arguments[0]);
                EnsureBoxed();
